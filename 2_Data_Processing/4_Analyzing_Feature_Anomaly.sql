...
SELECT location, date_, excess_mortality_cumulative_per_million AS original_values FROM data_load; -- Original data
-- Note: The subsequent query has apparently replaced all empty fields with zeroes when being type-casted:
SELECT location, date_, CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) AS type_casted_values -- This feature has been type-casted into a temporary table.
FROM data_load; 
...
-- 1)
SELECT location, date_, CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) AS char_to_num 
FROM data_load
...
SELECT location, date_, CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) AS char_to_num -- This query is congruent with the indicated note above, i.e., all empty fields were replaced with zeroes.
FROM data_load
...
SELECT location, date_, CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) AS non_zero_values -- Returns all values greater than 0, and all values less than 0.
FROM data_load                                                                                       		-- Anomaly! The values greater than and less than 0 are incorrect!
WHERE CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) > 0 OR CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) < 0
ORDER BY location; 

SELECT location, date_, CAST(excess_mortality_cumulative_per_million AS DECIMAL(30, 30)) AS non_zero_values, -- All values are between 1 and -1.
...
-- IMPORTANT! Be weary when type-casting features as the cause for this anomaly is unknown at this time.



-- Permanent-alteration Approaches to Resolve Anomaly Feature's Data-type Issue:

SELECT @@GLOBAL.sql_mode; -- Displays current arguments of the sql_mode variable; 
...
-- The following query may have to be run a second time for its execution to be complete:
